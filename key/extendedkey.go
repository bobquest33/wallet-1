...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	pubk, err := NewPublicKey(k.pubKeyBytes())
	if err != nil {
		return nil, err
	}
	_, parentFP := pubk.Address()
	parentFP = parentFP[:4]
...
func (k *ExtendedKey) Address() (*PublicKey, error) {
	return NewPublicKey(k.pubKeyBytes())
}
...
func (k *ExtendedKey) String() string {
	...
	if k.isPrivate {
		serializedBytes = append(serializedBytes, params.HDPrivateKeyID...)
	} else {
		serializedBytes = append(serializedBytes, params.HDPublicKeyID...)
	}
	...
	return base58check.Encode(serializedBytes[0], serializedBytes[1:])
}
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	payload, err := base58check.Decode(key)
	if err != nil {
		return nil, err
	}
